 ...
 * 
 * This script:
 * 1. Verifies one tournament-course mapping
 * 2. Runs the importer
 * 3. Reports row counts for all 9 normalized tables
 * 4. Validates the cascading data population
 */

import { default as prismaClient } from "../lib/prisma.js"
import { importCourseIntelligence } from "../lib/imports/course-intelligence-import.js"
...
async function main() {
  ...
  try {
    ...
    const firstMapping = await prisma.tournamentCourseMapping.findFirst({
    ...
    const updated = await prisma.tournamentCourseMapping.update({
    ...
    const result = await importCourseIntelligence(undefined, prisma)
...
async function getTableCounts() {
  return {
    course_details: await prisma.courseDetails.count(),
    course_addresses: await prisma.courseAddress.count(),
    course_coordinates: await prisma.courseCoordinates.count(),
    course_specifications: await prisma.courseSpecifications.count(),
    course_metadata: await prisma.courseMetadata.count(),
    playing_conditions: await prisma.playingConditions.count(),
    course_holes: await prisma.courseHole.count(),
    course_tees: await prisma.courseTee.count(),
    tee_hole_yardages: await prisma.teeHoleYardage.count(),
  }
}
