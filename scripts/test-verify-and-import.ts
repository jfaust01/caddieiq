...
async function main() {
  ...
  try {
    ...
    const candidates = await prisma.tournamentCourseMapping.findMany({
      where: {
        OR: [
          { tournamentCourseName: { contains: "Sawgrass" } },
          { tournamentCourseName: { contains: "Harbour Town" } },
          { tournamentCourseName: { contains: "Pebble Beach" } },
        ],
      },
      include: {
        tournamentCourse: {
    ...
    if (candidates.length === 0) {
      console.log("No TPC Sawgrass or Harbour Town found. Finding any high-confidence mapping...\n")
      const anyHigh = await prisma.tournamentCourseMapping.findFirst({
        where: {
      ...
      if (!anyHigh) {
        throw new Error("No mappings with >= 50% confidence found")
      }

      candidates.push(anyHigh)
    }

    const testMapping = candidates[0]
    ...
    console.log("✓ Was mapping selected? YES")
    console.log(`  - Mapping verified successfully`)
    console.log(`  - Now visible to importer\n`)

    console.log(`✓ Did course import execute? ${importResult.coursesImported > 0 ? "YES" : "PARTIAL"}`)
    console.log(`  - Courses considered: ${importResult.coursesConsidered}`)
    console.log(`  - Courses imported: ${importResult.coursesImported}\n`)

    const tablesWithRows = [
      { name: "courses", before: coursesBefore, after: coursesAfter },
      { name: "course_details", before: courseDetailsBefore, after: courseDetailsAfter },
      { name: "course_holes", before: courseHolesBefore, after: courseHolesAfter },
      { name: "course_tees", before: courseTeesBefore, after: courseTeesAfter },
      { name: "tee_hole_yardages", before: teeHoleYardagesBefore, after: teeHoleYardagesAfter },
    ].filter((t) => t.after > t.before)

    console.log(`✓ Which tables received rows? ${tablesWithRows.length > 0 ? "YES" : "NONE"}`)
    for (const table of tablesWithRows) {
      console.log(`  - ${table.name}: ${table.after - table.before} new rows`)
    }
    console.log("")

    console.log(`✓ Any downstream errors? ${importRuns.length > 0 && importRuns.some((r) => r.status === "ERROR") ? "YES" : "NO"}`)
