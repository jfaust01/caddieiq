    ...
    return (
      <div className="flex flex-col gap-0 w-full">
        {rounds.map((roundData, index) => (
          <div key={`round-${roundData.round}`} className="w-full py-0">
...
const RoundHoleRow = memo(function RoundHoleRow({
  ...
  return (
    <div className="flex items-center gap-2 w-full h-6">
      ...
      <div className="flex flex-col items-center justify-center gap-0 w-10 px-1.5 py-0.5 rounded border border-white/[0.1] bg-white/[0.03]">
        <div className="text-[7px] font-semibold text-gray-500 uppercase leading-none">
          R{round}
        </div>
        <div className={`text-[10px] font-bold tabular-nums leading-none ${toParColor}`}>
          {toParDisplay}
        </div>
      </div>
